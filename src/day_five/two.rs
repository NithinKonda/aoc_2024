...
impl Update {
    ...
    fn reorder_against_rules(&mut self, rules: &[Rule]) {
        loop {
            let mut reordered = false;
            for rule in rules {
                if !self.is_valid_against_rule(rule) {
                    let first_index = self.numbers.iter().position(|&x| x == rule.first).unwrap();
                    let second_index = self.numbers.iter().position(|&x| x == rule.second).unwrap();
                    self.numbers.swap(first_index, second_index);
                    reordered = true;
                }
            }
            if !reordered {
...
fn main() {
    ...
    for update in &mut updates {
        if !update.is_valid_against_rules(&rules) {
            update.reorder_against_rules(&rules);
        }
